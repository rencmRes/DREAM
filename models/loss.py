...
class RLDeconvolutionLoss(nn.Module):
    def __init__(self, psf_path):
        super(RLDeconvolutionLoss, self).__init__()
        self.psf = self.load_psf(psf_path)

    def load_psf(self, psf_path):
        psf_image = cv2.imread(psf_path, 0) 
        psf_image = psf_image / np.sum(psf_image)
        transform = transforms.ToTensor()
        psf_tensor = transform(psf_image).unsqueeze(0)
        return psf_tensor

    def forward(self, g, f):
        psf = self.psf.to(g.device) 
        hf_conv = F.conv2d(f, psf, padding='same')
        ratio = g / (hf_conv + 1e-7)
        h_transpose = torch.transpose(psf, 2, 3)
        term = ratio * h_transpose
        summed_term = torch.sum(term, dim=(1, 2, 3))
        loss = torch.mean((summed_term - 1) ** 2)
