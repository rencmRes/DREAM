...
class ModelBase():
    ...
    def load_network(self, load_path, network, strict=True, param_key='params'):
        ...
        if strict:
            ...
            loaded_params = {}
            state_dict = network.state_dict()
            for key, param_old in state_dict_old.items():
                if key in state_dict:
                    param = state_dict[key]
                    if param.shape == param_old.shape:
                        loaded_params[key] = param_old
                    else:
                        print(f"Shape mismatch for key '{key}': expected {param.shape}, got {param_old.shape}. Skipping.")
                else:
                    print(f"Key '{key}' not found in the current network. Skipping.")
            
            state_dict.update(loaded_params)
