...
class ModelGAN(ModelBase):
    ...
    def netG_forward(self, flag='testing'):
        if flag == 'Training':
            self.E, self.logits, self.labels = self.netG(self.L,flag)
        else:
            self.E = self.netG(self.L,flag)
    ...
    def optimize_parameters(self, current_step):
        ...
        self.netG_forward(flag='Training')
        loss_G_total = 0

        if current_step % self.D_update_ratio == 0 and current_step > self.D_init_iters:  # updata D first
            if self.opt_train['G_lossfn_weight'] > 0:
                G_loss_sr = self.G_lossfn_weight * self.G_lossfn(self.E, self.H)
                loss_constrast = self.contrast_loss(self.logits, self.labels)
                G_loss = G_loss_sr + loss_constrast
                loss_G_total += G_loss                 # 1) pixel loss
            if self.opt_train['F_lossfn_weight'] > 0:
                F_loss_sr = self.F_lossfn_weight * self.F_lossfn(self.E, self.H)
                F_loss = F_loss_sr 
        ...
        if current_step % self.D_update_ratio == 0 and current_step > self.D_init_iters:
            if self.opt_train['G_lossfn_weight'] > 0:
                ...
                self.log_dict['loss_constrast'] = loss_constrast.item()
            if self.opt_train['F_lossfn_weight'] > 0:
