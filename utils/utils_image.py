...
def imread_uint(path, n_channels=3):
    ...
    if n_channels == 1:
        img = cv2.imread(path, 0)  # BGR or G
        # img = np.array(Image.open(path), dtype=np.float32)  # cv2.IMREAD_GRAYSCALE
...
def imread_int(path, n_channels=3):
    #  input: path
    # output: HxWx3(RGB or GGG), or HxWx1 (G)
    if n_channels == 1:
...
def imsave(img, img_path):
    ...
    if img.ndim == 3:
        img = img[:, :, [2, 1, 0]]
    # high_quality_image = Image.fromarray(img)
    # high_quality_image.save(img_path)
